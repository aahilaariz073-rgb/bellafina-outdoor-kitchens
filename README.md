# BellaFina Outdoors — Outdoor Kitchens & Built-In BBQs

Single-page static landing site for **Outdoor Kitchens & Built-In BBQs**, deployed on Vercel at
**outdoorkitchens.bellafinaoutdoors.com**.

Complete outdoor kitchens lead, with built-in BBQs, grills and individual components
supporting — so the page catches both people planning a whole kitchen and people
shopping for one appliance.
...
3. Deploy
4. **Settings → Domains** → add `outdoorkitchens.bellafinaoutdoors.com`, then add the
...
It does not link to the other BellaFina landing sites (fire bowls, fire pits and pizza ovens).
The header nav is on-page anchors only:

`#top` Outdoor Kitchens · `#types` Components · `#bbqs` Built-In BBQs · `#who` Homeowners & Pros · `#areas` Service Area · `#faq` FAQ
...
`iD7GLxxCdv51i6umUCJF`. `<body data-lead-source="Outdoor kitchens and BBQ page">`
...
(`.hero-kitchen` in `assets/site.css`) and the split sections use gradient panels with an
...
- Point `outdoorkitchens.bellafinaoutdoors.com` at the Vercel project and confirm HTTPS
